...
if (isset($_GET['route'])) {
    ...
    if ($route == 'products') {
    ...
    } else if ($route == 'add_nalyt' && $request_method == 'POST') {
        $ip = $_SERVER['REMOTE_ADDR'];
        $added =  addAnalytic($ip);
        if ($added == false) {
            echo json_encode(['error' => true]);
        } else {
            echo json_encode(['id'  => $added]);
        }
        ...
        if ($data->lytic_id) {
            updateAnalyticMail($data->lytic_id, $data->full_name, $data->restrictions);
        }
...
function addAnalytic($ip) {

    global $conn;
    try {
        $query = "INSERT INTO views (ip ) VALUES (:ip )";
        $comment_query = $conn->prepare($query);
        $comment_query->bindParam(':ip', $ip);
        $comment_query->execute();
        $view_id = $conn->lastInsertId();
        unset($conn);
        return intval($view_id, 10);
...
function updateAnalyticMail($id, $n, $r) {
    global $conn;
    try {
        $query = "UPDATE views  SET name = :name, dietary  =  :dietary  WHERE id = :id";
        $comment_query = $conn->prepare($query);
        $comment_query->bindParam(':id', $id);
        $comment_query->bindParam(':name', $n);
        $comment_query->bindParam(':dietary', $r);
        $comment_query->execute();
        $view_id = $conn->lastInsertId();
        unset($conn);
        return intval($view_id, 10);
...
function updateAnalyticPurchased($id, $p) {
    global $conn;
    try {
        $query = "UPDATE views SET purchased_what = :purchased_what WHERE id = :id";
        $comment_query = $conn->prepare($query);
        $comment_query->bindParam(':id', $id);
        $comment_query->bindParam(':purchased_what', $p);
        $comment_query->execute();
        $view_id = $conn->lastInsertId();
        unset($conn);
        return intval($view_id, 10);
